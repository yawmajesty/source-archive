...
import {
  Camera, Megaphone, Plus, Trash2, Check, ChevronLeft, Save, Wand2, Calendar,
} from "lucide-react";
import {
  SHOOT_TYPES, SHOOT_STATUSES, REFERENCE_SLOTS, SLOT_LABEL, BRIEF_FIELDS,
  CREW_ROLES, PHASES, PHASE_LABEL, PHASE_TONE, CHANNELS, CHANNEL_LABEL,
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-4">
          <div className={`${CARD} p-4`}>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              <label className="block">
                <span className={LABEL}>Launch date</span>
                <input type="date" className={INPUT} defaultValue={campaign.launch_date ?? ""}
                  onBlur={(e) => void updateCampaign(campaign.id, { launch_date: e.target.value || null })} />
              </label>
              <label className="block sm:col-span-2">
                <span className={LABEL}>Who it's for</span>
                <input className={INPUT} placeholder="The audience you're actually talking to"
                  defaultValue={campaign.audience ?? ""}
                  onBlur={(e) => void updateCampaign(campaign.id, { audience: e.target.value })} />
              </label>
              <label className="block sm:col-span-3">
                <span className={LABEL}>What it has to achieve</span>
                <textarea className={`${INPUT} resize-y`} rows={2}
                  defaultValue={campaign.objective ?? ""}
                  onBlur={(e) => void updateCampaign(campaign.id, { objective: e.target.value })} />
              </label>
            </div>
          </div>

          {loading ? (
            <p className="text-[13px] text-[var(--sa-text-tertiary)]">Loading the plan…</p>
          ) : (
            phases.map((phase) => {
              ...
              return (
                <div key={phase.id} className={`${CARD} p-4`}>
                  <div className="flex flex-wrap items-baseline gap-2">
                    <span className="h-2.5 w-2.5 rounded-full" style={{ background: phase.tone }} />
                    <p className="text-[13px] font-semibold text-[var(--sa-text-primary)]">{phase.label}</p>
                    <span className="text-[11.5px] text-[var(--sa-text-tertiary)]">{phase.hint}</span>
                    <div className="flex-1" />
                    <button
                      onClick={async () => {
                        const res = await addCampaignItem({
                          campaignId: campaign.id, phase: phase.id as Phase,
                          channel: "instagram", title: "", position: rows.length,
                        });
                        if (res.success) setItems((p) => [...p, res.item]);
                      }}
                      className="flex items-center gap-1 text-[12px] font-medium text-[var(--sa-accent)]"
                    >
                      <Plus size={11} /> Add
                  ...
                  <div className="mt-2.5 flex flex-col gap-1.5">
                    {rows.length === 0 && (
                      <p className="text-[12px] text-[var(--sa-text-tertiary)]">Nothing here yet.</p>
                    )}
                    {rows.map((it) => (
                      <div key={it.id} className="group rounded-lg border border-[var(--sa-border)] p-2.5">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <input
                            className="min-w-[160px] flex-1 border-0 bg-transparent p-0 text-[13px] font-medium text-[var(--sa-text-primary)] outline-none"
                        ...
                        <textarea
                          className="mt-1 w-full resize-y border-0 bg-transparent p-0 text-[11.5px] leading-relaxed text-[var(--sa-text-tertiary)] outline-none"
                          rows={1}
                          placeholder="The brief for this one"
